...
// Madison Dockery: adapted from Proc C++
// 3/4/2020
...
template <typename Key, typename Value, 
  ...
  public:
    ...
    void rehash(size_t n);            // sets number of buckets to n and rehashes
  private:
...
hashmap<Key, Value, Compare, Hash>::insert(const Element& x) {
  ...
  if (it == elems_[bucket].end()) {
    ...
    // after element was inserted, find its iterator
    return make_pair(&(*findElement(x.first, bucket)),
		     true);
  }
  
  // element already existed so return it to elem
...
hashmap<Key, Value, Compare, Hash>::erase(const Key& x) {
  ...
  if (it != elems_[bucket].end()) {    // the element exists, erase it
    auto deleteThis = it; // storing iterator to be deleted
    ++it;                 // move iterator forward

    elems_[bucket].erase(deleteThis);
    --size_;

    // check to see if it's last element in bucket then move to next bucket
    if(it == elems_[bucket].end()) {
      // moving the iterator past empty buckets
      ++bucket;
      while(elems_[bucket].empty() && bucket != elems_.size())
	++bucket;

      if(bucket == elems_.size()) //checking if deleted the final element
	return make_pair(nullptr, true);

      else //move to the beginning of the next bucket
...
Value& hashmap<Key, Value, Compare, Hash>::operator[] (const Key& x) {
  ...
  if (found == nullptr) { // if key not found, create new element with empty value
    auto tmp = insert(make_pair(x, Value())); // calling default constructor on Value
    found = tmp.first;
  }
...
// sets the number of buckets in the container to n and rehashes

template<typename Key, typename Value, typename Compare, typename Hash>
void hashmap<Key, Value, Compare, Hash>::rehash(size_t n){
    
    // check if n is larger than number of buckets
  if(hash_.numBuckets() >= n)
    ...
    // copying old elements to a temp
    auto tmpBuckets = elems_;

    // creating a new list of elements
    elems_ = vector<list<Element>>(hash_.numBuckets());
